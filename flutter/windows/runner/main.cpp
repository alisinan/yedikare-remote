...
const char* uniLinksPrefix = "hoptodesk://";
...
{
  HINSTANCE hInstance = LoadLibraryA("libhoptodesk.dll");
  if (!hInstance)
  {
    std::cout << "Failed to load libhoptodesk.dll" << std::endl;
    return EXIT_FAILURE;
  }
  FUNC_RUSTDESK_CORE_MAIN hoptodesk_core_main =
      (FUNC_RUSTDESK_CORE_MAIN)GetProcAddress(hInstance, "hoptodesk_core_main_args");
  if (!hoptodesk_core_main)
  {
    std::cout << "Failed to get hoptodesk_core_main" << std::endl;
    return EXIT_FAILURE;
  }
  ...
  int args_len = 0;
  char** c_args = hoptodesk_core_main(&args_len);
  if (!c_args)
  {
    std::cout << "HopToDesk core returns false, exiting without launching Flutter app" << std::endl;
    return EXIT_SUCCESS;
  }
  ...
  // Uri links dispatch
  HWND hwnd = ::FindWindow(_T("FLUTTER_RUNNER_WIN32_WINDOW"), _T("YedikareRemote"));
  if (hwnd != NULL) {
  ...
  if (!window.CreateAndShow(
          is_cm_page ? L"YedikareRemote - Connection Manager" : L"YedikareRemote", origin,
          size, !is_cm_page)) {
      return EXIT_FAILURE;
  }
