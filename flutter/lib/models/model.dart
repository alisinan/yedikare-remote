...
class FfiModel with ChangeNotifier {
  ...
  toggleAutoRotate() {
    ...
    if (_autoRotate) {
      _updateOrientationForRemoteDisplay();
    } else {
...
*/
  ...
  handlePeerInfo(Map<String, dynamic> evt, String peerId, bool isCache) async {
    ...
    if (isPeerAndroid) {
      _touchMode = true;
      _gameMode = await bind.sessionGetOption(
    ...
    if (isMobile) {
      _autoRotate = await bind.sessionGetOption(
              sessionId: sessionId, arg: kOptionAutoRotate) !=
          '';
    }
