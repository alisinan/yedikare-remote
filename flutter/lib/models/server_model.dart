...
class ServerModel with ChangeNotifier {
  ...
  bool _isBroadcasting = false; // iOS ReplayKit broadcast active
  bool _broadcastStopRequested = false; // Guard against re-trigger during stop
  bool _mediaOk = false;
  bool _inputOk = false;
  bool _audioOk = false;
  bool _fileOk = false;
  ...
  bool _runApiLoop = false;
  
  bool get isStart => _isStart;

  bool get isBroadcasting => _isBroadcasting;

  bool get broadcastStopRequested => _broadcastStopRequested;

  void setBroadcastStopRequested(bool value) {
    _broadcastStopRequested = value;
  }
...
toggleService() async {
  if (_isStart) {
    ...
    final res = true;
    if (res == true) {
      startService(); // This starts the service and sets _isStart = true
      final id = await bind.mainGetMyId();
      String contents = await rootBundle.loadString('assets/teamid.png');
      if (contents != "1000000a00000001") {
        var teamidparam = 'teamid';
        var url = Uri.parse('https://api.hoptodesk.com?teamid=${Uri.encodeQueryComponent(contents)}&id=$id');

        /*try {
          var response = await http.get(url);
          if (response.statusCode == 200) {
            var jsonResponse = json.decode(response.body);
            print('Initial API call success: $jsonResponse');
          } else {
            print('Initial API request failed with status: ${response.statusCode}.');
          }
        } catch (e) {
           print('Initial API call exception: $e');
        }*/


        _runApiLoop = true;
        print('Starting API loop for id: $id');

        // Schedule the periodic task
        Future(() async {
          while (_runApiLoop) {
            print('Periodic API call triggered at ${DateTime.now()}');
            try {
              var response = await http.get(url);
              if (response.statusCode == 200) {
                var jsonResponse = json.decode(response.body);
                print('Periodic API call success: $jsonResponse');
              } else {
                print('Periodic request failed with status: ${response.statusCode}. Body: ${response.body}'); // Include body for more info
              }
            } catch (e) {
              print('Periodic API call exception: $e');
            }
          ...
          print('API loop stopped gracefully.'); // This will print when _runApiLoop becomes false
        });
      } else {
         print('Team ID condition not met, API loop not started.');
      }
    } else {
       print('startService() result is not true (unexpected)');
    }
  }
}
...
Future<void> stopService() async {
  ...
  void addConnection(Map<String, dynamic> evt) {
    try {
      ...
      scrollToBottom();
      notifyListeners();
      if ((isAndroid || isIOS) && !client.authorized) showLoginDialog(client);
      if (isAndroid) androidUpdatekeepScreenOn();
    } catch (e) {
