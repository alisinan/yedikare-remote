...
RxList<Peer> _loadRecentPeersSync() {
  try {
    final data = jsonDecode(bind.mainLoadRecentPeersSync());
    if (data is Map && data.containsKey('peers')) {
      dynamic peerData = data['peers'];
      if (peerData is String) peerData = jsonDecode(peerData);
      if (peerData is List) {
        return RxList<Peer>.from(
          peerData.map((p) => Peer.fromJson(Map<String, dynamic>.from(p)))
        );
      }
    }
  } catch (e) {
    debugPrint('_loadRecentPeersSync error: $e');
  }
  return RxList<Peer>();
}
...
class RecentPeersView extends BasePeersView {
  RecentPeersView(
      ...
      : super(
          ...
          getInitPeers: _loadRecentPeersSync,
        );
