...
const bool kIncludeInputControl =
    bool.fromEnvironment('INCLUDE_INPUT_CONTROL', defaultValue: false);
...
class _ServerPageState extends State<ServerPage> {
  ...
  void initState() {
    ...
    if (isIOS) {
      _setupIOSBroadcastListener();
    }
  }

  void _setupIOSBroadcastListener() {
    const channel = MethodChannel('com.hoptodesk.app/broadcast_picker');
    channel.setMethodCallHandler((call) async {
      debugPrint("iOS broadcast event: ${call.method}");
      switch (call.method) {
        case "broadcast_started":
          if (gFFI.serverModel.broadcastStopRequested) {
            debugPrint("iOS broadcast: ignoring broadcast_started during pending stop");
            break;
          }
          gFFI.serverModel.setBroadcasting(true);
          if (!gFFI.serverModel.isStart) {
            gFFI.serverModel.toggleService();
          }
          break;
        case "broadcast_stopped":
          gFFI.serverModel.setBroadcasting(false);
          if (gFFI.serverModel.isStart) {
            gFFI.serverModel.stopService();
          }
          Future.delayed(const Duration(seconds: 3), () {
            gFFI.serverModel.setBroadcastStopRequested(false);
          });
          break;
      }
      return null;
                  ...
                  child: Center(
                    child: Column(
                      ...
                      children: [
                        ...
                        if (!isIOS) const PermissionChecker(),
    ...
    return PaddingCard(
        child: Column(
          ...
          children: [
            ...
            Text(translate("Share Screen"),
                    style: const TextStyle(
                        fontSize: 18, fontWeight: FontWeight.bold))
                .marginOnly(bottom: 8),
            Text(translate(isIOS ? "ios_screen_share_tip" : "android_start_service_tip"),
                    textAlign: TextAlign.center,
                    style:
                        const TextStyle(fontSize: 13, color: MyTheme.darkGray))
                .marginOnly(bottom: 16),
            if (isIOS)
              const IOSBroadcastPickerButton()
            else
              ElevatedButton.icon(
...
class IOSBroadcastPickerButton extends StatelessWidget {
  static const _channel = MethodChannel('com.hoptodesk.app/broadcast_picker');

  const IOSBroadcastPickerButton({Key? key}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return SizedBox(
      ...
      child: ElevatedButton.icon(
        ...
        onPressed: () {
          _channel.invokeMethod('show_broadcast_picker');
        },
        label: Text(translate("Start Screen Share"),
...
class ServerInfo extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    Widget ConnectionStateNotification() {
      if (serverModel.connectStatus == -1) {
        ...
        return Row(children: [
          ...
          Expanded(child: Text(translate('connecting_status_short')))
        ]);
      } else {
        return Row(children: [
    ...
    return Column(children: [
      if (isIOS && serverModel.isBroadcasting)
        PaddingCard(
          child: Row(
            children: [
              Container(
              ...
              Expanded(
                child: Text(translate("Broadcasting your screen"),
                    style: const TextStyle(
                        fontSize: 15,
              ...
              SizedBox(
                ...
                child: ElevatedButton.icon(
                  ...
                  onPressed: () {
                    serverModel.setBroadcastStopRequested(true);
                    _channel.invokeMethod('stop_broadcast');
                    serverModel.stopService();
                  },
...
class _PermissionCheckerState extends State<PermissionChecker> {
  ...
  Widget build(BuildContext context) {
    final serverModel = Provider.of<ServerModel>(context);
    if (isIOS) {
      // iOS uses ReplayKit Broadcast Extension for screen sharing;
      // Android-style permission toggles are not applicable.
