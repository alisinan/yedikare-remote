...
class _RemotePageState extends State<RemotePage> with WidgetsBindingObserver {
 ...
 final TextEditingController _textController =
  ...
  List<TTextMenu> _getMobileActionMenus() {
    ...
    final enabled = versionCmp(gFFI.ffiModel.pi.version, '1.41.9') >= 0;
    if (!enabled) return [];
    return [
      TTextMenu(
      ...
      TTextMenu(
        child: Text(
          gFFI.ffiModel.gameMode
              ? translate('Game Mode (ON)')
              : translate('Game Mode (OFF)'),
        ),
        onPressed: () {
          setState(() {
            gFFI.ffiModel.toggleGameMode();
            final v = gFFI.ffiModel.gameMode ? 'Y' : '';
            bind.sessionPeerOption(
                sessionId: sessionId,
                name: kOptionGameMode,
                value: v);
          });
        },
      ),
