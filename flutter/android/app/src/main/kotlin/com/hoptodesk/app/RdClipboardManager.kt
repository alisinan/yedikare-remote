...
class RdClipboardManager(private val clipboardManager: ClipboardManager) {
	...
	fun rustUpdateClipboard(clips: ByteArray) {
		...
		// Add size check before attempting to set clipboard
		val MAX_CLIPBOARD_SIZE = 800 * 1024 // 800KB to be safe (limit is ~1MB)
		val textSize = text?.toByteArray(Charsets.UTF_8)?.size ?: 0
		val htmlSize = html?.toByteArray(Charsets.UTF_8)?.size ?: 0
		val totalSize = textSize + htmlSize
		
		if (totalSize > MAX_CLIPBOARD_SIZE) {
			Log.e(logTag, "Clipboard content too large: $totalSize bytes, max: $MAX_CLIPBOARD_SIZE bytes")
			return
		}
		...
		try {
			...
			if (e.cause is android.os.TransactionTooLargeException) {
				Log.e(logTag, "Clipboard content exceeded transaction size limit")
			}
		}
	}
}
