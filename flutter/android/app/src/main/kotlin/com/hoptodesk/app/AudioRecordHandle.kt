...
class AudioRecordHandle(private var context: Context, private var isVideoStart: ()->Boolean, private var isAudioStart: ()->Boolean) {
	...
	fun createAudioRecorder(inVoiceCall: Boolean, mediaProjection: MediaProjection?): Boolean {
		...
		if (inVoiceCall) {
			...
			mediaProjection?.let {
				try {
					...
					Log.w(logTag, "AudioPlaybackCaptureConfiguration failed, falling back to MIC: ${e.message}")
		...
		// Try to build with playback capture config first, fall back to MIC if it fails
		try {
			audioRecorder = builder.build()
		} catch (e: UnsupportedOperationException) {
			Log.w(logTag, "AudioRecord build failed with playback capture, falling back to MIC: ${e.message}")
			// Rebuild with MIC source as fallback
			...
			try {
				...
				Log.e(logTag, "Failed to create AudioRecord even with MIC source: ${e.message}")
				return false
			}
		}
