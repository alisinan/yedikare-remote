...
class MainService : Service() {
	...
	private fun invokeFlutterMethod(method: String, arguments: Any?) {
		Handler(Looper.getMainLooper()).post {
			MainActivity.flutterMethodChannel?.invokeMethod(method, arguments)
		}
	}
					...
					if (!inVoiceCall) {
						if (incomingVoiceCall) {
							...
							if (!audioRecordHandle.switchOutVoiceCall(mediaProjection)) {
								...
								invokeFlutterMethod("msgbox", mapOf(
    ...
    private val wakeLock: PowerManager.WakeLock by lazy { powerManager.newWakeLock(PowerManager.ACQUIRE_CAUSES_WAKEUP or PowerManager.SCREEN_BRIGHT_WAKE_LOCK, "hoptodesk:wakelock")}
    ...
    fun startCapture(): Boolean {
        ...
        mediaProjection!!.registerCallback(
            object : MediaProjection.Callback() {
                override fun onStop() {
                    Log.d(logTag, "callback: onStop")
                    super.onStop()
                }

                override fun onCapturedContentResize(width: Int, height: Int) {
                    Log.d(logTag, "callback: onCapturedContentResize " + width + "x" + height)
                }

                override fun onCapturedContentVisibilityChanged(isVisible: Boolean) {
                    Log.d(logTag, "callback: onCapturedContentVisibilityChanged $isVisible")
                }
            },
            serviceHandler
        )
    ...
    fun stopCapture() {
        ...
        // Remove listener before close to prevent callbacks from accessing freed image buffers
    ...
    private fun initNotification() {
        ...
        notificationChannel = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            val channelId = "HopToDesk"
            val channelName = "HopToDesk Service"
            val channel = NotificationChannel(
                channelId,
                channelName, NotificationManager.IMPORTANCE_HIGH
            ).apply {
                description = "HopToDesk Service Channel"
            }
	...
	private fun createForegroundNotification() {
		...
		Log.d(logTag, "Attempting to start foreground service")
		try {
			// For Android 14+, always start without specifying service type initially
			if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.UPSIDE_DOWN_CAKE) {
				startForeground(DEFAULT_NOTIFY_ID, notification)
			} else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q && mediaProjection != null) {
				Log.d(logTag, "Starting foreground service with mediaProjection type")
				startForeground(
					DEFAULT_NOTIFY_ID,
					notification,
				...
				Log.d(logTag, "Starting foreground service without specific type")
				startForeground(DEFAULT_NOTIFY_ID, notification)
			}
		} catch (e: SecurityException) {
			Log.e(logTag, "SecurityException caught: $e")
			// Fallback: Start without type
			try {
				startForeground(DEFAULT_NOTIFY_ID, notification)
			} catch (e2: Exception) {
				Log.e(logTag, "Failed to start foreground service: $e2")
				// If we can't start as foreground, we need to handle this gracefully
				stopSelf()
			}
		} catch (e: Exception) {
	...
	private fun loginRequestNotification(
	    ...
	    // Create a proper intent for the notification
	    val intent = Intent(this, MainActivity::class.java).apply {
	        flags = Intent.FLAG_ACTIVITY_NEW_TASK or 
	                Intent.FLAG_ACTIVITY_SINGLE_TOP or  // Add this
