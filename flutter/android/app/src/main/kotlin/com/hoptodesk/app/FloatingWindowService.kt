...
class FloatingWindowService : Service(), View.OnTouchListener {
	...
	override fun onDestroy() {
		super.onDestroy()
		handler.removeCallbacks(runnable)
		if (viewCreated && isViewAttachedToWindow(floatingView)) {
			try {
				windowManager.removeView(floatingView)
			} catch (e: IllegalArgumentException) {
				Log.d(logTag, "View already removed from window manager: $e")
			}
		}
		viewCreated = false // Reset the flag to avoid stale state
	}	
	...
	private fun isViewAttachedToWindow(view: View): Boolean {
		return try {
			...
			Log.d(logTag, "Error checking view attachment: $e")
			false
		}
	}
	...
	private fun moveToScreenSide(center: Boolean = false) {
		// Safety check: ensure view is still attached to window
		if (!viewCreated || !isViewAttachedToWindow(floatingView)) {
			Log.d(logTag, "View not attached to window, skipping moveToScreenSide")
			return
		}
		...
		try {
			...
			Log.d(logTag, "Failed to update view layout: $e")
			// Optionally recreate the view if needed
			// createView(windowManager)
		}
	}
    ...
    override fun onConfigurationChanged(newConfig: Configuration) {
     ...
     private fun showPopupMenu() {
         ...
         popupMenu.menu.add(0, idShowRustDesk, 0, translate("Show HopToDesk"))
