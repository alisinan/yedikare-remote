...
impl<T: InvokeUiSession> Session<T> {
    ...
    pub fn send_terminal_input(&self, terminal_id: i32, data: String) {
        ...
        td.data = bytes::Bytes::from(data.into_bytes());
        action.set_data(td);
        let mut msg_out = Message::new();
        msg_out.set_terminal_action(action);
        self.send(Data::Message(msg_out));
...
impl<T: InvokeUiSession> Interface for Session<T> {
    ...
    fn handle_peer_info(&self, mut pi: PeerInfo) {
        ...
        #[cfg(not(any(target_os = "android", target_os = "ios")))]
        if self.is_terminal() {
            // Send terminal open after connection is fully established.
            // Cannot rely on UI self.ready() timing as the sender channel
            // may not be set up yet when the page loads.
