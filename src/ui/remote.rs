...
struct MultiLayout {
    ...
    bbox_h: usize,
    composite: Vec<u8>,
    slots: HashMap<usize, (usize, usize, usize, usize)>,
}

lazy_static::lazy_static! {
    static ref VIDEO: Arc<Mutex<Option<Video>>> = Default::default();
    static ref MULTI_LAYOUT: Arc<Mutex<Option<MultiLayout>>> = Default::default();
}

// MCP dismiss signal: set by stdin reader thread, checked by Sciter timer
static MCP_DISMISS_FLAG: AtomicBool = AtomicBool::new(false);
// MCP query state flag: set by stdin reader, TIS gathers state and writes back
static MCP_QUERY_STATE_FLAG: AtomicBool = AtomicBool::new(false);

lazy_static::lazy_static! {
    // MCP click button command: button ID to click, consumed by Sciter timer
    ...
    // MCP debug output: TIS writes here, MCP reads for diagnostics
    static ref MCP_DEBUG: Mutex<String> = Mutex::new(String::new());
    // MCP state result: TIS writes full UI state JSON here, parent reads via temp file
    static ref MCP_STATE_RESULT: Mutex<String> = Mutex::new(String::new());
}

/// Start a background thread that reads commands from stdin (piped from parent process).
...
impl SciterHandler {
    ...
    pub fn ensure_terminal(&self, id: i32, rows: u16, cols: u16) {
        let mut map = self.terminals.lock().unwrap();
        match map.get_mut(&id) {
            Some(emu) => {
                if emu.size() != (rows, cols) {
                    emu.resize(rows, cols);
                }
            }
            None => {
                map.insert(id, TerminalEmulator::new(rows, cols));
            }
        }
    }

    pub fn drop_terminal(&self, id: i32) {
    ...
    fn dispatch_terminal_update(
        ...
        self.call(
            ...
            &make_args!(
                id,
                payload,
                update.cursor_row as i32,
                update.cursor_col as i32,
                update.cursor_visible,
                update.app_cursor_keys,
                update.app_keypad,
                update.alt_screen,
                update.bracketed_paste,
                update.full_repaint,
    ...
    pub fn get_scrollback_view(&self, id: i32, offset: usize) -> Value {
        let mut map = self.terminals.lock().unwrap();
        let emu = match map.get_mut(&id) {
            Some(e) => e,
            None => return Value::null(),
        };
        let update = match std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            || emu.set_view(offset),
        )) {
            Ok(u) => u,
        ...
        let mut v = Value::map();
        v.set_item("payload", payload);
        v.set_item("cur_row", update.cursor_row as i32);
        v.set_item("cur_col", update.cursor_col as i32);
        v.set_item("cur_vis", update.cursor_visible);
        v.set_item("app_cur", update.app_cursor_keys);
        v.set_item("app_kp", update.app_keypad);
        v.set_item("alt", update.alt_screen);
        v.set_item("bracketed", update.bracketed_paste);
        v.set_item("full", update.full_repaint);
        v.set_item("title", update.title.clone().unwrap_or_default());
        v
    }
...
impl InvokeUiSession for SciterHandler {
    ...
    fn set_display(&self, x: i32, y: i32, w: i32, h: i32, cursor_embedded: bool) {
        ...
        if MULTI_LAYOUT.lock().unwrap().is_some() {
            return;
        }
    ...
    fn on_rgba(&self, display: usize, rgba: &mut scrap::ImageRgb) {
        let mut layout_lock = MULTI_LAYOUT.lock().unwrap();
        if let Some(layout) = layout_lock.as_mut() {
            if let Some(&(slot_x, slot_y, slot_w, slot_h)) = layout.slots.get(&display) {
                let bbox_w = layout.bbox_w;
                let bbox_h = layout.bbox_h;
                if slot_x + slot_w <= bbox_w && slot_y + slot_h <= bbox_h {
                    let src_stride = slot_w * 4;
                    let dst_stride = bbox_w * 4;
                    if rgba.raw.len() >= src_stride * slot_h {
                        let composite = &mut layout.composite;
                        for row in 0..slot_h {
                            let src_off = row * src_stride;
                            let dst_off = (slot_y + row) * dst_stride + slot_x * 4;
                            composite[dst_off..dst_off + src_stride]
                                .copy_from_slice(&rgba.raw[src_off..src_off + src_stride]);
                        }
                        if let Some(v) = VIDEO.lock().unwrap().as_mut() {
                            v.render_frame(composite).ok();
                        }
                    }
                }
            }
    ...
    fn set_peer_info(&self, pi: &PeerInfo) {
        ...
        pi_sciter.set_item("dashboard_linked", pi.dashboard_linked);
    ...
    fn on_connected(&self, conn_type: ConnType) {
        match conn_type {
            ConnType::DEFAULT_CONN => {
                #[cfg(target_os = "macos")]
                {
                    if !crate::platform::macos::is_can_input_monitoring(true) {
                        log::warn!("Input Monitoring permission denied, keyboard may not work");
                        self.msgbox(
    ...
    fn portable_service_running(&self, running: bool) {
        self.call("portableServiceRunning", &make_args!(running));
    }
    ...
    fn handle_terminal_response(&self, resp: TerminalResponse) {
        ...
        match resp.union {
            Some(Union::Opened(opened)) => {
                self.call("terminalOpened", &make_args!(opened.terminal_id, opened.success, opened.message));
            }
            ...
            Some(Union::Closed(closed)) => {
                ...
                self.call("terminalClosed", &make_args!(closed.terminal_id, closed.exit_code));
            }
            Some(Union::Error(error)) => {
                self.call("terminalError", &make_args!(error.terminal_id, error.message));
            }
            _ => {}
        }
    }
...
impl SciterSession {
    fn send_terminal_open(&self, terminal_id: i32, rows: i32, cols: i32) {
        self.session.ui_handler.ensure_terminal(
            terminal_id,
            rows.max(1) as u16,
            cols.max(1) as u16,
        );
        self.session.send_terminal_open(terminal_id, rows, cols);
    }

    fn send_terminal_resize(&self, terminal_id: i32, rows: i32, cols: i32) {
        self.session.ui_handler.ensure_terminal(
            terminal_id,
            rows.max(1) as u16,
            cols.max(1) as u16,
        );
        self.session.send_terminal_resize(terminal_id, rows, cols);
    }

    fn send_terminal_close(&self, terminal_id: i32) {
        self.session.ui_handler.drop_terminal(terminal_id);
        self.session.send_terminal_close(terminal_id);
    }

    fn view_terminal_scrollback(&self, terminal_id: i32, offset: i32) -> Value {
        let n = if offset < 0 { 0 } else { offset as usize };
        self.session.ui_handler.get_scrollback_view(terminal_id, n)
    }
}
...
impl sciter::EventHandler for SciterSession {
    ...
    fn attached(&mut self, root: HELEMENT) {
        let el = Element::from(root);
        *self.element.lock().unwrap() = Some(el.clone());
        #[cfg(any(target_os = "windows", target_os = "macos", target_os = "linux"))]
        crate::platform::pinch_zoom::register_remote_view(el);
    }

    fn detached(&mut self, _root: HELEMENT) {
        *self.element.lock().unwrap() = None;
        #[cfg(any(target_os = "windows", target_os = "macos", target_os = "linux"))]
        crate::platform::pinch_zoom::unregister_remote_view();
    ...
    sciter::dispatch_script_call! {
        ...
        fn is_rdp();
        fn is_terminal();
        fn send_terminal_open(i32, i32, i32);
        fn send_terminal_input(i32, String);
        fn send_terminal_close(i32);
        fn send_terminal_resize(i32, i32, i32);
        fn view_terminal_scrollback(i32, i32);
        ...
        fn switch_display_set(Value);
        fn set_multi_layout(i32, i32, Value);
        ...
        fn send_key_to_remote(i32, String, i32);
        fn get_size();
        ...
        fn has_dashboard_link();
        fn request_add_to_dashboard();
...
impl SciterSession {
    pub fn new(cmd: String, id: String, password: String, tokenexp: String, args: Vec<String>) -> Self {
        ...
        // Start stdin command reader for MCP dismiss_dialog support
    ...
    fn switch_display(&self, display: i32) {
        *MULTI_LAYOUT.lock().unwrap() = None;
        self.session.switch_display(display);
    }

    fn switch_display_set(&self, displays: Value) {
        let mut idxs: Vec<i32> = Vec::new();
        for i in 0..displays.len() {
            if let Some(v) = displays.get(i).to_int() {
                idxs.push(v);
            }
        }
        self.session.capture_displays(vec![], vec![], idxs);
    }

    fn set_multi_layout(&self, bbox_w: i32, bbox_h: i32, slots: Value) {
        let bw = bbox_w.max(0) as usize;
        let bh = bbox_h.max(0) as usize;
        if bw == 0 || bh == 0 {
            *MULTI_LAYOUT.lock().unwrap() = None;
            if let Some(v) = VIDEO.lock().unwrap().as_mut() {
                v.stop_streaming().ok();
        ...
        for i in 0..slots.len() {
            let s = slots.get(i);
            if s.len() >= 5 {
                let idx = s.get(0).to_int().unwrap_or(0).max(0) as usize;
                let x = s.get(1).to_int().unwrap_or(0).max(0) as usize;
                let y = s.get(2).to_int().unwrap_or(0).max(0) as usize;
                let w = s.get(3).to_int().unwrap_or(0).max(0) as usize;
                let h = s.get(4).to_int().unwrap_or(0).max(0) as usize;
                map.insert(idx, (x, y, w, h));
            }
        }
        let composite = vec![0u8; bw * bh * 4];
        *MULTI_LAYOUT.lock().unwrap() = Some(MultiLayout {
            bbox_w: bw,
            bbox_h: bh,
            composite,
            slots: map,
        ...
        if let Some(v) = VIDEO.lock().unwrap().as_mut() {
            v.stop_streaming().ok();
            v.start_streaming((bw as i32, bh as i32), COLOR_SPACE::Rgb32, None).ok();
        }
    }
    ...
    fn alternative_codecs(&self) -> Value {
        let (vp8, h264, h265) = self.session.alternative_codecs();
        let mut v = Value::array(0);
        v.push(vp8);
        v.push(h264);
        v.push(h265);
    ...
    fn is_wayland_no_grab(&self) -> bool {
        #[cfg(target_os = "linux")]
        { return !crate::platform::linux::is_x11(); }
        #[cfg(not(target_os = "linux"))]
        { return false; }
    }

    fn send_key_to_remote(&self, down_or_up: i32, name: String, code: i32) {
        if let Some(mut key_event) = self.get_key_event(down_or_up, &name, code) {
            if down_or_up == 0 {
                key_event.down = true;
            }
            crate::keyboard::send_key_event(&key_event);
        }
    }
    ...
    fn switch_sides_handler(&mut self) {
        self.switch_sides();
    }
    ...
    fn remote_print(&self) {
        // Handled from TIScript via showPrinterDialog()
    }
    ...
    fn get_default_printer(&self) -> String {
        #[cfg(any(target_os = "windows", target_os = "macos"))]
        {
            crate::platform::get_default_printer().unwrap_or_default()
        }
        #[cfg(not(any(target_os = "windows", target_os = "macos")))]
        {
            String::new()
    ...
    fn on_printer_selected(&self, id: i32, _path: String, printer_name: String) {
        use crate::ui_session_interface::take_print_job;
        use hbb_common::config::{keys, LocalConfig};

        log::info!("on_printer_selected called: id={}, printer={}", id, printer_name);

        // Check if "always use this printer" should be saved
        // The UI should call this with a special flag or we handle it differently
        // For now, we'll save the printer name if it's selected
        if !printer_name.is_empty() {
            LocalConfig::set_option(keys::OPTION_PRINTER_SELECTED_NAME.to_string(), printer_name.clone());
        }

        // Get pending print data and print it
        #[cfg(any(target_os = "windows", target_os = "macos"))]
        {
            if let Some(data) = take_print_job(id) {
                log::info!("Found print job data: {} bytes, sending to printer...", data.len());
                let printer = if printer_name.is_empty() { None } else { Some(printer_name) };
                std::thread::spawn(move || {
                    match crate::platform::send_raw_data_to_printer(printer, data) {
                        Ok(()) => log::info!("Print job completed successfully"),
                        Err(e) => log::error!("Print job error: {}", e),
                    }
                });
            } else {
                log::error!("No pending print data found for job id {}", id);
            }
        }
    }

    fn check_mcp_dismiss(&self) -> bool {
        MCP_DISMISS_FLAG.swap(false, Ordering::SeqCst)
    }

    fn get_mcp_click_button(&self) -> String {
        let mut lock = MCP_CLICK_BUTTON.lock().unwrap();
        let val = lock.clone();
        lock.clear();
        val
    }

    fn get_mcp_chat_msg(&self) -> String {
        let mut lock = MCP_CHAT_MSG.lock().unwrap();
        let val = lock.clone();
        lock.clear();
        val
    }

    fn get_mcp_password(&self) -> String {
        let mut lock = MCP_PASSWORD.lock().unwrap();
        let val = lock.clone();
        lock.clear();
        val
    }

    fn check_mcp_query_state(&self) -> bool {
        MCP_QUERY_STATE_FLAG.swap(false, Ordering::SeqCst)
    }

    fn write_mcp_state(&self, peer_id: String, state_json: String) {
        *MCP_STATE_RESULT.lock().unwrap() = state_json.clone();
        // Write to temp file so parent process can read it
        let path = std::env::temp_dir().join(format!("hoptodesk-mcp-state-{}.json", peer_id));
        let _ = std::fs::write(&path, &state_json);
    }

    fn set_mcp_debug(&self, msg: String) {
        *MCP_DEBUG.lock().unwrap() = msg;
    }

    fn get_mcp_debug(&self) -> String {
        MCP_DEBUG.lock().unwrap().clone()
    }
}
