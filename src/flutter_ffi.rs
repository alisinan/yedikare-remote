...
pub fn main_set_option(key: String, value: String) {
    ...
    if key.eq("custom-rendezvous-server") {
        ...
        crate::common::test_rendezvous_server();
    } else if key.eq("custom-api-url") {
        set_option(key, value.clone());
        std::thread::spawn(|| {
            if let Ok(rt) = hbb_common::tokio::runtime::Runtime::new() {
                rt.block_on(hbb_common::api::erase_api());
            }
        });
        #[cfg(target_os = "android")]
        crate::rendezvous_mediator::RendezvousMediator::restart();
...
pub fn main_set_socks(proxy: String, username: String, password: String) {
    set_socks(proxy, username, password, "auto".to_string())
}
