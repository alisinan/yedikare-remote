...
impl<T: RelayConnObserver + Send + Sync> Conn for RelayConn<T> {
    ...
    async fn send_to(&self, _: &[u8], addr: SocketAddr) -> Result<usize, util::Error> {
        let mut relay_conn = self.relay_conn.lock().await;
        match relay_conn.create_permission(addr).await {
            Ok(n) => Ok(n),
...
impl<T: RelayConnObserver + Send + Sync> RelayConnInternal<T> {
    ...
    // see SetDeadline and SetWriteDeadline.
    // On packet-oriented connections, write timeouts are rare.
    async fn create_permission(&mut self, addr: SocketAddr) -> Result<usize, Error> {
