...
fn generate_bindings(
    ...
    match b.generate() {
        Ok(bindings) => {
            ...
            // Check if bindgen produced opaque types (broken bindings)
            let content = fs::read_to_string(ffi_rs).unwrap_or_default();
            if content.contains("pub _address") && exact_file.exists() {
                eprintln!("Note: bindgen produced opaque types, using pre-generated bindings");
